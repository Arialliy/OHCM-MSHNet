...
if [[ -f "$OUT/eval_full/summary_metrics.json" \
   && -f "$OUT/eval_hcset/summary_metrics.json" \
...
printf 'method=%s dataset=%s seed=%s gpu=%s\n' "$METHOD" "$DATASET" "$SEED" "$GPU" > "$LOCK_DIR/task"
...
python tools/export_step0_predictions.py \
  ...
  --checkpoint "$OUT/checkpoints/$DATASET/${MODEL_NAME}_${EPOCHS}.pth.tar" \

...
OUT_IN_CONTAINER="$ROOT_IN_CONTAINER/results/step5_experiments/$RUN_ID/$METHOD/$DATASET/seed_$SEED"
...
container_name="ohcm_step5_${name_method}_${name_dataset}_s${SEED}_${RUN_ID}"
...
docker run --rm \
  ...
  bash -lc "set -euo pipefail
    ...
    python train.py \
      ...
      $TRAIN_EXTRA_ARGS \
      2>&1 | tee '$OUT_IN_CONTAINER/train_console.log'
    ...
    if [[ -f '$ROOT_IN_CONTAINER/results/step0_baseline/20260611_155232/step2_hcset/hcset_${DATASET}.txt' ]]; then
      python tools/evaluate_prediction_exports.py \
        --dataset_dir ./datasets \
        --dataset_name '$DATASET' \
        --train_dataset_name '$DATASET' \
        ...
        --image_list '$ROOT_IN_CONTAINER/results/step0_baseline/20260611_155232/step2_hcset/hcset_${DATASET}.txt' \
        --method '$METHOD' \
        --seed '$SEED' \
        --threshold '$THRESHOLD' \
    ...
    python tools/analyze_step1_hard_clutter.py \
      ...
      --dataset_name '$DATASET' \
      --exports_dir '$OUT_IN_CONTAINER/exports' \
      --output_dir '$OUT_IN_CONTAINER/step1_fp_analysis' \
      --seed '$SEED' \
      --threshold '$THRESHOLD' \
      2>&1 | tee '$OUT_IN_CONTAINER/step1_fp_analysis_console.log'

...
def random_crop_with_coords(img, mask, patch_size, pos_prob=None):
    h, w = img.shape
    ...
    if pos_prob is None or cur_prob > pos_prob or mask.max() == 0:
        ...
        loc = np.where(mask > 0)
        idx = 0 if len(loc[0]) <= 1 else random.randint(0, len(loc[0]) - 1)
        h_start = random.randint(max(0, loc[0][idx] - patch_size), min(loc[0][idx], h - patch_size))
        w_start = random.randint(max(0, loc[1][idx] - patch_size), min(loc[1][idx], w - patch_size))
    ...
    return img[h_start:h_end, w_start:w_end], mask[h_start:h_end, w_start:w_end], coords


def crop_by_coords(arr, coords, patch_size=None):
    h_start, h_end, w_start, w_end = coords
    out = arr[h_start:h_end, w_start:w_end]
    if patch_size is not None and out.shape != (patch_size, patch_size):
        pad_h = patch_size - out.shape[0]
        pad_w = patch_size - out.shape[1]
        out = np.pad(out, ((0, max(0, pad_h)), (0, max(0, pad_w))), mode='constant')
    return out


def apply_aug_ops(arr, ops):
    if ops[0]:
        arr = arr[::-1, :]
    if ops[1]:
        arr = arr[:, ::-1]
...
def get_img_norm_cfg(dataset_name, dataset_dir):
    if  dataset_name == 'NUAA-SIRST':
        ...
        train_list_path = dataset_dir + '/' + dataset_name + '/img_idx/train_' + dataset_name + '.txt'
        if not os.path.exists(train_list_path):
            raise FileNotFoundError(
                'Cannot infer img_norm_cfg: missing train split %s. '
                'Provide --img_norm_cfg_mean/--img_norm_cfg_std for blind or external datasets.'
                % train_list_path
            )
        ...
        img_list = train_list
        if not img_list:
            raise ValueError('Cannot infer img_norm_cfg: empty train split %s.' % train_list_path)
...
def get_optimizer(net, optimizer_name, scheduler_name, optimizer_settings, scheduler_settings):
    trainable_parameters = [param for param in net.parameters() if param.requires_grad]
    if not trainable_parameters:
        raise ValueError("No trainable parameters found for optimizer.")
    if optimizer_name == 'Adam':
        optimizer = torch.optim.Adam(trainable_parameters, lr=optimizer_settings['lr'])
    elif optimizer_name == 'Adagrad':
        optimizer  = torch.optim.Adagrad(trainable_parameters, lr=optimizer_settings['lr'])
    elif optimizer_name == 'SGD':
        optimizer  = torch.optim.SGD(trainable_parameters, lr=optimizer_settings['lr'])

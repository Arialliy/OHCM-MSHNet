...
def load_bcv_evidence_checkpoint(net, checkpoint_path, device):
    checkpoint = torch_load_checkpoint(checkpoint_path, device)
    ...
    if missing_evidence:
        raise RuntimeError(f"Missing BCV evidence keys: {missing_evidence[:8]}")
    if unexpected:
        raise RuntimeError(f"Unexpected BCV evidence keys: {unexpected[:8]}")
    net.eval()
...
def main():
    parser = argparse.ArgumentParser(description="BCV Gate-A beta=0 equivalence audit against MSHNetOHEM.")
    ...
    parser.add_argument("--bcv_beta_max", type=float, default=0.1)
    parser.add_argument("--bcv_hidden_channels", type=int, default=32)
    parser.add_argument("--bcv_evidence_threshold", type=float, default=0.0)
    parser.add_argument("--bcv_contrast_kernel", type=int, default=9)
    ...
    loader = DataLoader(
        TestSetLoader(args.dataset_dir, train_dataset_name, args.dataset_name, img_norm_cfg),
        num_workers=1,
        batch_size=1,
        shuffle=False,
    )
    ...
    bcv_cfg = dict(vars(args))
    bcv_cfg["bcv_eval_beta"] = 0.0
    bcv = Net("BCVMSHNet", mode="test", loss_cfg=bcv_cfg).to(device)
    checkpoint = load_ohem_checkpoint(ohem, args.checkpoint, device)
    load_bcv_evidence_checkpoint(bcv, args.checkpoint, device)

    ohem_stats, bcv_stats = init_stats(), init_stats()
    rows = []
    ...
    with torch.no_grad():
        for idx, (img, gt_mask, size, image_name) in enumerate(loader):
            ...
            bcv_export = bcv.export_logits_features(img)
            bcv_logit = bcv_export["logit"]
            suppression = bcv_export["suppression_map"]
            ohem_prob = foreground_probability(ohem_logit)[0, 0, :h, :w].detach().cpu().numpy().astype(np.float32)
            bcv_prob = foreground_probability(bcv_logit)[0, 0, :h, :w].detach().cpu().numpy().astype(np.float32)
            gt = gt_mask[0, 0, :h, :w].numpy() > 0
            ohem_mask, bcv_mask = ohem_prob > args.threshold, bcv_prob > args.threshold
            prob_diff = float(np.max(np.abs(ohem_prob - bcv_prob)))
            mask_diff = int(np.not_equal(ohem_mask, bcv_mask).sum())
            ...
            update_stats(bcv_stats, bcv_prob, gt, args.threshold)
            rows.append({"image_name": name, "max_prob_diff": prob_diff, "mask_diff_pixels": mask_diff, "suppression_max": suppression_max})
            if (idx + 1) % 100 == 0:
                print(f"Gate-BCV-A checked [{idx + 1}/{len(loader)}]", flush=True)

    ohem_metrics, bcv_metrics = finalize_stats(ohem_stats), finalize_stats(bcv_stats)
    metric_diff = {key: abs(ohem_metrics[key] - bcv_metrics[key]) for key in ("mIoU", "Pd", "FA_ppm")}
    checks = {
    ...
    summary = {
        "gate": "BCV_Gate_A_beta0_equivalence",

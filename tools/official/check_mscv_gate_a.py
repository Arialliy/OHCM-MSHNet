...
def load_mscv_evidence_checkpoint(net, checkpoint_path, device):
    checkpoint = torch_load_checkpoint(checkpoint_path, device)
    ...
    if missing_evidence:
        raise RuntimeError(f"Missing MSCV evidence keys: {missing_evidence[:8]}")
    if unexpected:
        raise RuntimeError(f"Unexpected MSCV evidence keys: {unexpected[:8]}")
    net.eval()
...
def main():
    parser = argparse.ArgumentParser(description="MSCV Gate-A beta=0 equivalence audit against MSHNetOHEM.")
    ...
    mscv = Net("MSCVMSHNet", mode="test", loss_cfg=mscv_cfg).to(device)
    checkpoint = load_ohem_checkpoint(ohem, args.checkpoint, device)
    load_mscv_evidence_checkpoint(mscv, args.checkpoint, device)

    ohem_stats, mscv_stats = init_stats(), init_stats()
    rows = []
    ...
    with torch.no_grad():
        for idx, (img, gt_mask, size, image_name) in enumerate(loader):
            ...
            mscv_export = mscv.export_logits_features(img)
            mscv_logit = mscv_export["logit"]
            suppression = mscv_export["suppression_map"]
            ohem_prob = foreground_probability(ohem_logit)[0, 0, :h, :w].detach().cpu().numpy().astype(np.float32)
            mscv_prob = foreground_probability(mscv_logit)[0, 0, :h, :w].detach().cpu().numpy().astype(np.float32)
            gt = gt_mask[0, 0, :h, :w].numpy() > 0
            ohem_mask, mscv_mask = ohem_prob > args.threshold, mscv_prob > args.threshold
            prob_diff = float(np.max(np.abs(ohem_prob - mscv_prob)))
            mask_diff = int(np.not_equal(ohem_mask, mscv_mask).sum())
            ...
            update_stats(mscv_stats, mscv_prob, gt, args.threshold)
            rows.append({"image_name": name, "max_prob_diff": prob_diff, "mask_diff_pixels": mask_diff, "suppression_max": suppression_max})
            if (idx + 1) % 100 == 0:
                print(f"Gate-MSCV-A checked [{idx + 1}/{len(loader)}]", flush=True)

    ohem_metrics, mscv_metrics = finalize_stats(ohem_stats), finalize_stats(mscv_stats)
    metric_diff = {key: abs(ohem_metrics[key] - mscv_metrics[key]) for key in ("mIoU", "Pd", "FA_ppm")}
    checks = {
    ...
    summary = {
        "gate": "MSCV_Gate_A_beta0_equivalence",
